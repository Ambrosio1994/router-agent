...
def get_memory_content(config: RunnableConfig, store: BaseStore):
    ...
    if existing_memory:
        return existing_memory.value.get('memory')  # Retorna o conteúdo da memória se existir
    else:
...
def router_node(state: State):
    ...
    """
    agent = llm_router.invoke([router_prompt] + state["messages"])  # Invoca o modelo de roteamento
...
def code_node(state: State, config: RunnableConfig, store: BaseStore):
    ...
    system_msg = code_prompt.format(memory=memory_content)  # Formata o prompt com a memória
    resp = llm_code.invoke([system_msg] + state["messages"])  # Invoca o modelo de geração de código
...
def thinking_node(state: State, config: RunnableConfig, store: BaseStore):
    ...
    """
    memory_content = get_memory_content(config, store)  # Obtém a memória do usuário
    system_msg = thinking_prompt.format(memory=memory_content)  # Formata o prompt com a memória
    resp = llm_thinking.invoke([system_msg] + state["messages"])  # Invoca o modelo com capacidade de raciocínio
    return {"response": resp.content[1]["text"]}  # Retorna o texto de pensamento processado
...
def simple_node(state: State, config: RunnableConfig, store: BaseStore):
    ...
    """
    memory_content = get_memory_content(config, store)  # Obtém a memória do usuário
    system_msg = simple_prompt.format(memory=memory_content)  # Formata o prompt com a memória
...
def build_graph():
    ...
    builder.add_conditional_edges("router", choose_agent)  # Escolhe o agente baseado na decisão do roteador
    builder.add_edge("router", "write_memory")  # Conecta o roteador à escrita de memória
